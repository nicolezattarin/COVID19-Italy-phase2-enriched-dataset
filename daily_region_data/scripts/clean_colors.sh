#!/bin/sh
# This bash script creates a slightly altered copy of the colors_daily dataset for the fix_region
# script. The starting colors_daily file is the same file that is in the ../../region_colors directory.
rm cleaned_colors_daily.csv
tail -n 171 colors_daily.csv | sed -e 's/,/ /g' >> cleaned_colors_daily.csv
# Where 171 is the number of records in the colors_daily dataset.
